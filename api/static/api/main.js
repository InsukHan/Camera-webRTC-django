...
function takeSnapshot() {
  ...
  function getFileName(imageNumber) {
    const prefix = 'photo';
    const photoNumber = padWithZeroNumber(imageNumber, 4);
    return prefix + '-' + photoNumber + '.jpg';
  }
  ...
  getCanvasBlob(canvas).then(function (blob) {
    ...
    console.log(form);
    const filename = getFileName(imageNumber);
    let file = new File([blob], filename);
    form.append('image',file);
    console.log(form);
    //form.submit();
    imageNumber++;
    console.log('form append success!',file.size,file.name);
    $.ajax({
      url: '',
      type: 'POST',
      data: form,
